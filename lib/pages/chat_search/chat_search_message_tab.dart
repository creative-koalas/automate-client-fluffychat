...
class _MessageSearchResultListTile extends StatelessWidget {
  ...
  ({Uri? avatarUrl, String displayName}) _resolveSenderPresentation(
    BuildContext context,
    User user,
  ) {
    final matrixLocals = MatrixLocals(L10n.of(context));
    final agentService = AgentService.instance;
    final fallbackDisplayName = user.calcDisplayname(i18n: matrixLocals);

    agentService.ensureMatrixProfilePresentationById(
      client: room.client,
      matrixUserId: user.id,
      fallbackDisplayName: user.displayName ?? fallbackDisplayName,
      fallbackAvatarUri: user.avatarUrl,
    );

    return (
      avatarUrl: agentService.resolveAvatarUriByMatrixUserId(
            user.id,
            fallbackAvatarUri: user.avatarUrl,
          ) ??
          user.avatarUrl,
      displayName: resolveDisplayNameForMatrixUserId(
        room: room,
        matrixUserId: user.id,
